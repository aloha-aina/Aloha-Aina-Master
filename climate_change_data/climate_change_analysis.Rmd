...
## read in daily avg climate data exported from jupyter notebook, code can be found here: https://github.com/aloha-aina/Aloha-Aina-Master 
```{r}
## daily avgs
historical = read.csv(here("climate_change_data","daily_avg","oahu_historical_1850_2014_avg.csv")) %>%  
  rename(date = dayofyear, pcp = pr) 
ssp126 = read.csv(here("climate_change_data","daily_avg","oahu_ssp126_2015_2100_avg.csv")) %>% 
  rename(date = dayofyear, pcp = pr) 
ssp245 = read.csv(here("climate_change_data","daily_avg","oahu_ssp245_2015_2100_avg.csv")) %>% 
  rename(date = dayofyear, pcp = pr) 
ssp370 = read.csv(here("climate_change_data","daily_avg","oahu_ssp370_2015_2100_avg.csv")) %>% 
  rename(date = dayofyear, pcp = pr)
ssp585 = read.csv(here("climate_change_data","daily_avg","oahu_ssp585_2015_2100_avg.csv")) %>% 
  rename(date = dayofyear, pcp = pr)

## daily totals
historical_tot = read.csv(here("climate_change_data","daily_tot","oahu_historical_1850_2014_total.csv")) %>%  
  rename(date = dayofyear, pcp = pr) 
ssp126_tot = read.csv(here("climate_change_data","daily_tot","oahu_ssp126_2015_2100_total.csv")) %>% 
  rename(date = dayofyear, pcp = pr) 
ssp245_tot = read.csv(here("climate_change_data","daily_tot","oahu_ssp245_2015_2100_total.csv")) %>% 
  rename(date = dayofyear, pcp = pr) 
ssp370_tot = read.csv(here("climate_change_data","daily_tot","oahu_ssp370_2015_2100_total.csv")) %>% 
  rename(date = dayofyear, pcp = pr)
ssp585_tot = read.csv(here("climate_change_data","daily_tot","oahu_ssp585_2015_2100_total.csv")) %>% 
  rename(date = dayofyear, pcp = pr)
...
```{r}
## separate historical total precip based on 50 yr time periods
historical_tot_1900_1950 <- historical_tot %>% 
  filter(year %in% (1900:1950))
  
## separate projections based on 2020 to 2070
ssp126_tot_2020_2070 <- ssp126_tot %>% 
  filter(year %in% (2020:2070))

ssp245_tot_2020_2070 <- ssp245_tot %>% 
  filter(year %in% (2020:2070))

ssp370_tot_2020_2070 <- ssp370_tot %>% 
  filter(year %in% (2020:2070))

ssp585_tot_2020_2070 <- ssp585_tot %>% 
  filter(year %in% (2020:2070))


daily_tot_period <- 
  ggplot() +
  geom_line(data = ssp126_tot_2020_2070, aes(x=date, y=pcp, color="orange"), size = 0.5) + 
  geom_line(data = ssp245_tot_2020_2070, aes(x=date, y=pcp, color="red"), size = 0.5) +
  geom_line(data = ssp370_tot_2030_2070, aes(x=date, y=pcp, color = "blue"), size = 0.5) +
  geom_line(data = ssp585_tot_2020_2070, aes(x=date, y=pcp, color = "seagreen"), size = 0.5) + 
  theme_minimal() + 
  scale_y_continuous(breaks=seq(0,50,10)) +
  scale_x_date(date_breaks = "3 years", date_labels = "%Y") +
  ggtitle("Oahu, HI Daily Total Precipitation, CNRM-ESM2-1: 2020-2070") +
   labs(x = "Year",
       y = "Precipitation (in)") +
  scale_color_manual(values = c("orange", "red", "blue", "seagreen"), labels = c("spp126", "ssp245", "ssp370", "ssp585"))+
   theme(legend.title=element_blank())+
...
daily_tot_period_hist <- 
  ggplot() +
  geom_line(data = historical_tot_1900_1950, aes(x=date, y=pcp, color="purple"), size = 0.5) +  
  theme_minimal() + 
  scale_y_continuous(breaks=seq(0,50,10)) +
  scale_x_date(date_breaks = "3.5 years", date_labels = "%Y") +
  ggtitle("Oahu, HI Daily Total Precipitation, CNRM-ESM2-1, historical: 1900-1950 ") +
   labs(x = "Year",
       y = "Precipitation (in)") +
  scale_color_manual(values = c("purple"), labels = c("historical")) + 
 theme(legend.title=element_blank()) +
    theme(legend.position="bottom")
...
```{r}
## first lets merge the data frames together 
daily_tot_prec_merge1 <- ssp126_tot_2020_2070 %>% right_join(ssp245_tot_2020_2070, by = c("date")) 
daily_tot_prec_merge2 <- daily_tot_prec_merge1 %>%  right_join(ssp370_tot_2020_2070, by = c("date"))
daily_tot_prec_merge_fin <- daily_tot_prec_merge2 %>% right_join(ssp585_tot_2020_2070,by = c("date")) %>% 
...
historical_tot_1900_1950_rename <- historical_tot_1900_1950 %>% 
...
merge_his_proj <- cbind(daily_tot_prec_merge_fin, historical_tot_1900_1950_rename) 
