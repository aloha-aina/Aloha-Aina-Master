...
ggplot() +
  ...
  ggtitle("Kuliouou discharged cfs")
```

## filter for 2010 and 2009 range
```{r}
...
## plot 2009 Kuliouou daily total stream discharge
```{r}
kuli_discharge_2009_daily <- kuli_discharge_2009 %>%
  group_by(date) %>%
  summarize(
    daily_discharge = sum(discharge_ft3_s))
...
## plot 2010 Kuliouou daily summed stream discharge
```{r}
...
## identifying possible storm to match with precip and input into swmm
```{r}
kuli_discharge_2009 %>% 
ggplot() +
  geom_line(aes(x=date, y=discharge_ft3_s), color="darkseagreen") +
  ...
  labs(x="Time (hours)", y="Discharge (cfs)") +
  scale_x_date(date_breaks = "1 month")  + 
  ggtitle("Kuliouou 2009 subhourly discharged cfs")

## possbile 2009 storms:
# 2009-09-07
# 2009-10-11
# 2009-10-20
# 2009-11-02
...
## 2009-09-07 storm event 
kuli_discharge_sep10 <- kuli_discharge %>% 
 filter(date >= as.Date("2009-09-05") & date <= as.Date("2009-09-10"))

kuli_discharge_sep10 %>% 
ggplot() +
  geom_point(aes(x=date, y=discharge_ft3_s), color="darkseagreen") +
  theme_classic() +
  labs(x="Time (hours)", y="Discharge (cfs)")  
  ggtitle("Kuliouou Sep 04-10 2009 subhourly discharged cfs")

## 2009-09-07 storm event 
kuli_discharge_oct11 <- kuli_discharge %>% 
 filter(date >= as.Date("2009-10-09") & date <= as.Date("2009-10-12"))

kuli_discharge_oct11 %>% 
ggplot() +
  geom_point(aes(x=date, y=discharge_ft3_s), color="darkseagreen") +
  theme_classic() +
  labs(x="Time (hours)", y="Discharge (cfs)")   + 
