...
## PDF code sourced from: https://rstudio-pubs-static.s3.amazonaws.com/100906_8e3a32dd11c14b839468db756cee7400.html
```{r}
...
```
## PDF with the scaled march 14, 2009 storm
```{r}
...
## plot density distribution plot
pdf_plot_density_corrected <- ggplot(data=dStandardNormal, aes(x=daily_pcp, y= Density)) +
  geom_line(size=1.5) +
  theme_classic() +
  labs(x= "Daily Precipitation (in)") +
  geom_vline(xintercept = 1.462713, linetype="solid",  ## 2.20 (Max MCF) * 0.6648695 = 1.462713
                color = "#D55E00", size=1.5) +
  geom_vline(xintercept = 0.2659478, linetype="solid",  ## 0.40 (Min MCF) * 0.6648695 = 0.2659478
                color = "#009E73", size=1.5) +
  # annotate("text", x = 4.75, y = 0.75, label = "Max MCF * avg pcp", color = "#D55E00", size =5) +
  # annotate("text", x = 4.75, y = 0.70, label = "Min MCF * avg pcp", color = "#009E73",size =5) +
    theme(text = element_text(size=22)) +
...
pdf_plot_density_corrected


## plot density distribution plot
pdf_plot_density_corrected2 <- ggplot(data=dStandardNormal, aes(x=daily_pcp, y= Density)) +
  geom_line(size=1.5) +
  theme_classic() +
  labs(x= "Daily Precipitation (in)") +
