...
daily_tot_period <- 
  ...
  ggtitle("Oahu, HI Daily Total Precipitation, CNRM-ESM2-1: 2020-2050") +
   labs(x = "Year",
       y = "Precipitation (in)") +
  scale_color_manual(values = c("orange", "red", "blue", "seagreen"), labels = c("spp126", "ssp245", "ssp370", "ssp585"))+
   theme(legend.title=element_blank())+
...
ggsave('cnrm_daily_tot_period.png', daily_tot_period, width = 12, height = 10, units = "in")
```
...
## look at analogeous storm event we will use for swimm 2009-10-11
```{r}
...
ggplot(data = merge_his_proj_filt, aes(x=year, y=fut_hist_ratio,  group=year)) +
  geom_boxplot() +
  facet_wrap(~ssp) +
  theme_minimal() +
  labs(x="Year", y="Future Projections/Historical Ratio (Oct 11)")
  #scale_y_continuous(limits = c(0,0.0e+00)
