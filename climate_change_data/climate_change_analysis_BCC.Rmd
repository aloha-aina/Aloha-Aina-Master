...
bcc_daily_tot_period <- 
  ggplot() +
  geom_line(data = bcc_historical_tot_1984_2014, aes(x=date, y=pcp, color="black"), size = 0.5) + 
  ...
  ggtitle("Oahu, HI Daily Total Precipitation, BCC-CSM2-MR: 1984-2050") +
   labs(x = "Year",
       y = "Precipitation (in)") +
...
#ggsave('bcc_daily_tot_period.png', bcc_daily_tot_period, width = 12, height = 8, units = "in")

```

## historical 50 year period graph
```{r}
bcc_daily_tot_period_hist <- 
  ggplot() +
  geom_line(data = bcc_historical_tot_1984_2014, aes(x=date, y=pcp, color="black"), size = 0.5) +  
  theme_minimal() + 
  scale_y_continuous(breaks=seq(0,50,10)) +
  scale_x_date(date_breaks = "3.5 years", date_labels = "%Y") +
  ggtitle("Oahu, HI Daily Total Precipitation, BCC-CSM2-MR, historical: 1984-2014 ") +
   labs(x = "Year",
       y = "Precipitation (in)") +
...
bcc_daily_tot_prec_merge1 <- bcc_ssp126_tot_2020_2050 %>% right_join(bcc_ssp245_tot_2020_2050, by = c("date")) 
bcc_daily_tot_prec_merge2 <- bcc_daily_tot_prec_merge1 %>%  right_join(bcc_ssp370_tot_2020_2050, by = c("date"))
bcc_daily_tot_prec_merge_fin <- bcc_daily_tot_prec_merge2 %>% right_join(bcc_ssp585_tot_2020_2050,by = c("date")) %>% 
...
bcc_historical_tot_1984_2014_rename <- bcc_historical_tot_1984_2014 %>% 
...
bcc_merge_his_proj <- cbind(bcc_daily_tot_prec_merge_fin, bcc_historical_tot_1984_2014_rename) 

bcc_ratio_df <- bcc_merge_his_proj %>% 
...
ggplot(data = bcc_ratio_df, aes(x=date, y=fut_hist_ratio,  group=ssp)) +
  geom_boxplot() +
  facet_wrap(~ssp)
...
bcc_merge_his_proj_filt <- bcc_merge_his_proj %>% 
  filter(month == "10") %>% 
  filter(day == "11") %>% 
  mutate(ssp126_hist = ssp126_pcp/hist_pcp,
         ssp245_hist = ssp245_pcp/hist_pcp,
...
proj_hist_oct11_BCC <- ggplot(data = bcc_merge_his_proj_filt, aes(x=year, y=fut_hist_ratio,  group=year)) +
  geom_boxplot() +
  facet_wrap(~ssp, labeller = as_labeller(ssp_names)) +
  theme_minimal() +
  labs(x="Year", 
       y="Future Projection/Historical Daily Total Ratio (Oct 11)", 
       caption = "Data Source: BCC-CSM2-MR") +
  scale_y_continuous(limits = c(0,30)) +
    theme(text = element_text(size=15))

proj_hist_oct11_BCC

#ggsave('proj_hist_oct11_BCC.png', proj_hist_oct11_BCC, width = 12, height = 10, units = "in")
...
```{r}
bcc_merge_his_proj_filt_dec19 <- bcc_merge_his_proj %>%
  filter(month == "12") %>%
  filter(day == "19") %>%
  group_by(month,day) %>%
...
bcc_merge_his_proj_filt_dec19 <- bcc_merge_his_proj_filt_dec19 %>%
...
proj_hist_dec19_bcc <- ggplot(data = bcc_merge_his_proj_filt_dec19, aes(x=ssp, y=fut_hist_ratio)) +
  geom_point(size=2) +
  theme_minimal() +
  labs(x="Shared Socioeconomic Pathways (ssp)",
       y="Future Projection/Historical Daily Total Average Ratio (Dec 19)",
       caption = "Climate Model: BCC-CSM2-MR") +
    theme(text = element_text(size=15)) +
...
proj_hist_dec19_bcc
 ...
 # ggsave('proj_hist_dec19_bcc.png', proj_hist_dec19_bcc, width = 12, height = 10, units = "in")

```

## March 14, 2009 = Another Wailupe storm that we will use to do CC analysis with. 
```{r}
bcc_merge_his_proj_filt_mar14 <- bcc_merge_his_proj %>%
  filter(month == "3") %>%
  filter(day == "14") %>%
  group_by(month,day) %>%
  mutate(ssp126_pcp_avg = mean(ssp126_pcp),
...
bcc_merge_his_proj_filt_mar14 <- bcc_merge_his_proj_filt_mar14 %>%
  mutate(ssp126_hist = ssp126_pcp_avg/hist_pcp_avg,
         ssp245_hist = ssp245_pcp_avg/hist_pcp_avg,
         ssp370_hist = ssp370_pcp_avg/hist_pcp_avg,
         ssp585_hist = ssp585_pcp_avg/hist_pcp_avg) %>%
...
proj_hist_mar14_bcc <- ggplot(data = bcc_merge_his_proj_filt_mar14, aes(x=ssp, y=fut_hist_ratio)) +
  geom_point(size=2) +
  theme_minimal() +
  labs(x="Shared Socioeconomic Pathways (ssp)",
...
proj_hist_mar14_bcc
 ...
 # ggsave('proj_hist_mar14_bcc.png', proj_hist_mar14_bcc, width = 12, height = 10, units = "in")
