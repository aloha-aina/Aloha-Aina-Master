...
paiko_nov_2009 <- paiko_daily_76 %>% 
  filter(year == 2009) %>%
  filter(date >= "2009-11-29" & date <= "2009-12-04")
...
## filter wailupe rain 77 daily to range of kuli stream gage 
```{r}
wailupe77_2009_2010 <- wailupe_daily_77 %>% 
  filter(date >= "2009-07-09" & date <= "2010-10-03")

ggplot() +
  geom_line(data = wailupe77_2009_2010, aes(x = date,
                                         y = daily_pcp,
                                         color = "red")) +
  geom_point()+
  ...
  theme_minimal()
```
## filter paiko rain 77 daily to range of kuli stream gage 
```{r}
paiko_2009_2010 <- paiko_daily_76 %>% 
  filter(date >= "2009-07-09" & date <= "2010-10-03")

ggplot() +
  geom_line(data = paiko_2009_2010, aes(x = date,
                                         y = daily_pcp,
                                         color = "red")) +
  geom_point()+
  scale_x_date(date_breaks = "2 month") +
...
```{r}
wailupe77_2009_12_03 <- wailupe_tidy_77 %>% 
  filter(date >= "2009-12-02" & date <= "2009-12-04")

ggplot() +
  geom_line(data = wailupe77_2009_12_03, aes(x = time,
                                         y = qpcp,
                                         color = "red")) +
  theme_minimal()
...
```{r}
wailupe77_2009_10_11 <- wailupe_tidy_77 %>% 
  filter(date == c("2009-10-11"))

ggplot() +
  geom_point(data = wailupe77_2009_10_11, aes(x = time,
                                         y = qgag,
                                         color = "red")) +
  theme_minimal()
...
```
## Climate change Multiplicative Factor (MCF) applied to March 14th, 2009 storm event (see climate_change_analysis_IPSL.Rmd in climate_change_data folder for final MCF values used)
```{r}
wailupe77_2009_03_14 <- wailupe_tidy_77 %>% 
  filter(date == c("2009-03-14")) %>% 
  select(station_name,date,time,qpcp) %>% 
  mutate(qpcp_MCF_min = 0.4043945*qpcp,
         qpcp_MCF_max =  2.1998642*qpcp) 

wailupe77_2009_03_14$qpcp_MCF_min <- round(wailupe77_2009_03_14$qpcp_MCF_min  ,2)
wailupe77_2009_03_14$qpcp_MCF_max <- round(wailupe77_2009_03_14$qpcp_MCF_max ,2)

wailupe77_2010_12_19 <- wailupe_tidy_08 %>% 
  filter(date == c("2010-12-19")) %>% 
  select(station_name,date,time,qpcp) %>% 
  mutate(qpcp_MCF_min = 0.1850539*qpcp,
         qpcp_MCF_max =  3.0046226*qpcp) 

wailupe77_2010_12_19$qpcp_MCF_min <- round(wailupe77_2010_12_19$qpcp_MCF_min  ,2)
wailupe77_2010_12_19$qpcp_MCF_max <- round(wailupe77_2010_12_19$qpcp_MCF_max ,2)
